...
| **4 — End-to-end Model** | Validate dynamic and fixed-`r=2` selection through the OpenAI-compatible V3 API | 2×A800-80GB GPUs, 512 GB host memory; reserve 8–16 visible physical CPU cores for the system | 398.17 GiB RESplit | About 20 min |
...
```bash
./scripts/ae/container_ae.sh build
./scripts/ae/container_ae.sh mwe
```

The commands below document the corresponding Docker operations and allow each
step to be inspected or run independently.
...
```bash
./scripts/ae/container_ae.sh pull
```
...
```bash
./scripts/ae/container_ae.sh prepare-config
```
...
```bash
./scripts/ae/container_ae.sh prepare-one-layer
./scripts/ae/container_ae.sh exp2
```
...
```bash
./scripts/ae/container_ae.sh exp3
```
...
The wrapper prepares the full model, performs the strict preflight, starts one
mode, waits for readiness, runs the API check and stops the server:

```bash
./scripts/ae/container_ae.sh prepare-full
./scripts/ae/container_ae.sh exp4 dynamic
./scripts/ae/container_ae.sh exp4 static-r2
```

The following commands show the same stages explicitly.
