...
class KDeepseekV3ForCausalLM(DeepseekV3PreTrainedModel):
    ...
    def forward_flex(
        ...
        for i, decode_layer in enumerate(self.model.layers):
            if i <= self.config.first_k_dense_replace:
                ...
                hs_bs1 = hidden_states[bs_tokens:,:]
    ...
    def forward(
        ...
        for i, decode_layer in enumerate(self.model.layers):
            # Expert Deferral: 进入本层时 sync 上一层 MoE 的 phase1+phase2 并合并到 hidden_states
            if i > 0 and (i - 1) >= self.config.first_k_dense_replace and (i - 1) % self.config.moe_layer_freq == 0:
                ...
                if hasattr(prev_mlp, "experts") and hasattr(prev_mlp.experts, "generate_experts"):
                    ...
                    if getattr(ge, "expert_deferral_enabled", False):
                        ge.sync_and_add_pending_to(hidden_states, cuda_graph_idx)
