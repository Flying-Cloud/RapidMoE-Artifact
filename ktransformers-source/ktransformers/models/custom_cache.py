...
class KDeepSeekV3Cache(nn.Module):
    ...
    def load(self, inference_context: "sched_ext.InferenceContext"): 
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            # DeepSeek-V3 Hybrid-2 places layers 0--30 on cuda:0 and 31--60
            # on cuda:1. Keep each layer's paged KV cache beside its attention
            # module so CUDA Graph capture never mixes devices.
            device = "cuda:1" if i > 30 else "cuda:0"
            self.k_caches.append(
                inference_context.k_cache[0][i].to(device)
            )
