        ...
        if child is not None:
            ...
            if child_prefix in local_optimization_dict:
                ...
                if inject_module_meta["class"] != "default":
                    ...
                    # 为 KExpertsHybrid 添加 dynamic_topk 参数（如果未在 kwargs 中指定）
                    kwargs = inject_module_meta["kwargs"] if "kwargs" in inject_module_meta else dict()
                    if import_class_name == "KExpertsHybrid" and "dynamic_topk" not in kwargs:
                        kwargs["dynamic_topk"] = Config().dynamic_topk if hasattr(Config(), 'dynamic_topk') else True
                    inject_module=module_cls(key = inject_module_meta["key"], gguf_loader = gguf_loader, config = model_config, orig_module=child, **kwargs)
