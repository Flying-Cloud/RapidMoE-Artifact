...
static __device__ __forceinline__ float vec_dot_iq2_xxs_q8_k128_shared(
    const void * __restrict__ vbq, const block_q8_K128 * __restrict__ bq8_1, const int & iqs, const uint64_t * __restrict__ shared_iq2xxs_grid, const uint8_t * __restrict__ shared_ksigns_iq2xs) {
    const block_iq2_xxs * bq2 = (const block_iq2_xxs *) vbq;
    ...
    int sumi = 0;
    for (int l = 0; l < 4; ++l) {
        const uint8_t * grid = (const uint8_t *)(shared_iq2xxs_grid + aux8[l]);
        const uint8_t  signs = shared_ksigns_iq2xs[aux32 & 127];
        for (int j = 0; j < 8; ++j) {
            sumi += q8[j] * grid[j] * (signs & kmask_iq2xs[j] ? -1 : 1);
        }
        q8 += 8;
...
static __device__ __forceinline__ float vec_dot_iq1_s_r4_q8_k128_shared(
    const void * __restrict__ vbq, const block_q8_K128 * __restrict__ bq8_1, const int & iqs, const int & iqa, const float & dx, const uint64_t * __restrict__ shared_iq1s_grid) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
    const block_iq1_s_r4 * bq1 = (const block_iq1_s_r4 *) vbq;

    //const int       qs_packed = get_int_b2(bq1->qs, iqs);
    //const uint8_t * qs        = (const uint8_t *) &qs_packed;
    const uint8_t * qs        = bq1->qs;
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        const int grid = shared_iq1s_grid[qs[l0*2+iqs] | (((qh >> 3*(l0/2)) & 0x07) << 8)];

        const int grid0 = (grid >> 0) & 0x0F0F0F0F;
        const int grid1 = (grid >> 4) & 0x0F0F0F0F;

        const int u0 = get_int_b4(bq8_1[0].qs, 8*iqa + l0 + 0);
        const int u1 = get_int_b4(bq8_1[0].qs, 8*iqa + l0 + 1);
        //printf("threadIdx.x = %d, u0 = %d, u1 = %d\n", threadIdx.x, u0, u1);
        sumi = __dp4a(grid0, u0, sumi);
        sumi = __dp4a(grid1, u1, sumi);
    }
    // dx正常
    // qh不正常 bsums[iqa]不好说
    // printf("threadIdx.x = %d, iqs = %d, iqa = %d, dx = %f, sumi = %d, qh = %d, bq8_1[0].bsums[iqa] = %d\n", threadIdx.x, iqs, iqa, dx, sumi, qh, bq8_1[0].bsums[iqa]);
    // printf("threadIdx.x = %d, bq8_1[0].d = %f, bq8_1[0].bsums[iqa] = %d\n", threadIdx.x, bq8_1[0].d, bq8_1[0].bsums[iqa]);
    const float  d1q   = dx * (((qh >> 11) & 0x0E) + 1);
    const float  delta = -1.0f + IQ1S_DELTA - (qh & 0x8000) * (2.0f*IQ1S_DELTA/0x8000);
    const float  bsum = (float)bq8_1[0].bsums[iqa];
    return d1q * bq8_1[0].d*(sumi + bsum*delta);
#endif
}
static __device__ __forceinline__ float vec_dot_iq1_m_r4_q8_k128_shared(
    const void * __restrict__ vbq, const block_q8_K128 * __restrict__ bq8_1, const int & iqs, const int & iqa, const float & dx, const uint64_t * __restrict__ shared_iq1s_grid) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
    const block_iq1_m_r4 * bq1 = (const block_iq1_m_r4 *) vbq;
    // iqs代表输出行索引
    //const int       qs_packed = get_int_b2(bq1->qs, iqs);
    //const uint8_t * qs        = (const uint8_t *) &qs_packed;
    const uint8_t * qs        = bq1->qs;
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        const int l1   = l0 / 4;
        const int qhl  = bq1->qh[l1*4 + iqs] >> (4 * ((l0/2) % 2));
        const int grid = shared_iq1s_grid[qs[l0*2+iqs] | ((qhl & 0x07) << 8)];

        const int grid0 = (grid >> 0) & 0x0F0F0F0F;
        const int grid1 = (grid >> 4) & 0x0F0F0F0F;
        //const int grid1 = 0x0F0F0F0F;

        const int u0 = get_int_b4(bq8_1[0].qs, 8*iqa + l0 + 0);
        const int u1 = get_int_b4(bq8_1[0].qs, 8*iqa + l0 + 1);
        //printf("threadIdx.x = %d, u0 = %d, u1 = %d\n", threadIdx.x, u0, u1);
        sumi[l1] = __dp4a(grid0, u0, sumi[l1]);
        sumi[l1] = __dp4a(grid1, u1, sumi[l1]);
    ...
    // printf("threadIdx.x = %d, bq8_1[0].d = %f, bq8_1[0].bsums[iqa] = %d\n", threadIdx.x, bq8_1[0].d, bq8_1[0].bsums[iqa]);
    // scale的低四位对应前8个数的scale，高8位对应后八个数的scale
    const float  d1q = dx * (bq1->scales[iqs] & 0xf);
    const float  d2q = dx * (bq1->scales[iqs] >> 4);
    return bq8_1[0].d * ((sumi[0] + sumf[0]) * d1q +(sumi[1] + sumf[1]) * d2q);
#endif
}

template <int batch_size>
static __device__ __forceinline__ void vec_dot_iq1_m_r4_q8_k128_shared_batch(
    const void * __restrict__ vbq,
    const block_q8_K128 * const * __restrict__ bq8_ptrs,
    const bool * __restrict__ active,
    const int & iqs, const int & iqa, const float & dx,
    const uint64_t * __restrict__ shared_iq1s_grid,
    float * __restrict__ out) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
    const block_iq1_m_r4 * bq1 = (const block_iq1_m_r4 *) vbq;
    const uint8_t * qs = bq1->qs;

    int sumi[batch_size][2];
    float sumf[batch_size][2];
    #pragma unroll
    ...
    const int qhl_t0  = bq1->qh[iqs];
    const int qhl_t1  = bq1->qh[iqs] >> 4;
    const int qhl_t2  = bq1->qh[4 + iqs];
    const int qhl_t3  = bq1->qh[4 + iqs] >> 4;
    const int grid_t0 = shared_iq1s_grid[qs[iqs] | ((qhl_t0 & 0x07) << 8)];
    const int grid_t1 = shared_iq1s_grid[qs[iqs] | ((qhl_t1 & 0x07) << 8)];
    const int grid_t2 = shared_iq1s_grid[qs[8 + iqs] | ((qhl_t2 & 0x07) << 8)];
    const int grid_t3 = shared_iq1s_grid[qs[8 + iqs] | ((qhl_t3 & 0x07) << 8)];
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        const int l1 = l0 / 4;
        const int qhl = bq1->qh[l1*4 + iqs] >> (4 * ((l0/2) % 2));
        ...
        for (int c = 0; c < batch_size; ++c) {
            ...
            const int u0 = get_int_b4(bq8_ptrs[c][0].qs, 8*iqa + l0 + 0);
            const int u1 = get_int_b4(bq8_ptrs[c][0].qs, 8*iqa + l0 + 1);
            sumi[c][l1] = __dp4a(grid0, u0, sumi[c][l1]);
            sumi[c][l1] = __dp4a(grid1, u1, sumi[c][l1]);
            int sumy = 0;
            sumy = __dp4a(u0, 0x01010101, sumy);
            sumy = __dp4a(u1, 0x01010101, sumy);
            sumf[c][l1] += delta * sumy;
        }
    }
    ...
    for (int c = 0; c < batch_size; ++c) {
        if (!active[c]) continue;
        out[c] += bq8_ptrs[c][0].d * ((sumi[c][0] + sumf[c][0]) * d1q + (sumi[c][1] + sumf[c][1]) * d2q);
    }
#endif
}
static __device__ __forceinline__ float vec_dot_iq1_m_r4_q8_k128_shared_16b(
    const void * __restrict__ vbq, const block_q8_K128 * __restrict__ bq8_1, const int & iqs, const int & iqa, const float & dx, 
    const uint16_t * __restrict__ shared_iq1s_grid) // 注意：这里将传入参数改为了 uint16_t *
{
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
    const block_iq1_m_r4 * bq1 = (const block_iq1_m_r4 *) vbq;
    const uint8_t * qs        = bq1->qs;
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        ...
        // 1. 读取 16-bit 压实后的网格数据
        const uint16_t grid16 = shared_iq1s_grid[qs[l0*2+iqs] | ((qhl & 0x07) << 8)];

        // 2. 将数据分为属于 grid0 (低 8 位) 和 grid1 (高 8 位) 两部分
        const int x0 = grid16 & 0xFF;       
        const int x1 = grid16 >> 8;         

        // 3. 核心解压魔法：将 4 个相邻的 2-bit 值，并发散开到 4 个字节(8-bit)的最低 2 位上
        // 通过自身位移并求并集，然后统一用 0x03 (即 00000011) 洗掉中间重叠的垃圾数据位
        const int grid0 = (x0 | (x0 << 6) | (x0 << 12) | (x0 << 18)) & 0x03030303;
        const int grid1 = (x1 | (x1 << 6) | (x1 << 12) | (x1 << 18)) & 0x03030303;
...
static __device__ __forceinline__ float vec_dot_iq1_m_r4_q8_k128_shared_opt(
    const void * __restrict__ vbq, const block_q8_K128 * __restrict__ bq8_1, const int & iqs, const int & iqa, const float & dx, const uint64_t * __restrict__ shared_iq1s_grid) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
    const block_iq1_m_r4 * bq1 = (const block_iq1_m_r4 *) vbq;
    const uint8_t * qs = bq1->qs;
    
    // 1. 独立累加器，打破 dp4a 数据依赖 (Ampere 的常规操作)
    int sumi0_0 = 0, sumi0_1 = 0; // l1 = 0 时的累加结果
    int sumi1_0 = 0, sumi1_1 = 0; // l1 = 1 时的累加结果

    // 注意：所有的 sumy 和 sumy_weight 变量已被彻底删除！

    // 2. Q8 激活值的 32-bit 安全访存 (绕过 16-byte 不对齐的硬件报错)
    const int* q8_ptr = reinterpret_cast<const int*>(bq8_1[0].qs);
    const int base_idx = 8 * iqa;

    // --- 第一次展开 (l0 = 0, l1 = 0) ---
    const int u0 = __ldg(&q8_ptr[base_idx + 0]); 
    const int u1 = __ldg(&q8_ptr[base_idx + 1]); 
    
    // 取低 4 位，最高位直接作为 delta(-7 或 -9) 的索引
    const int qhl_0 = bq1->qh[iqs] & 0x0F;
    // 查出 64-bit 的值，里面已经包含 8 个乘过 8 并加了 delta 的 signed 8-bit 整数！
    // const uint64_t grid_val_0 = shared_iq1s_grid[qs[iqs] | (qhl_0 << 8)];
    const uint64_t grid_val_0 = __ldg(&iq1s_grid_gpu_new[qs[iqs] | (qhl_0 << 8)]);
    // 直接强转截断，省去了所有的 & 0x0F0F0F0F 掩码操作
    const int grid0_0 = (int)(grid_val_0 & 0xFFFFFFFF);
    const int grid0_1 = (int)(grid_val_0 >> 32);

    // 发射核心计算 (只需 2 次！)
    sumi0_0 = __dp4a(grid0_0, u0, sumi0_0);
    sumi0_1 = __dp4a(grid0_1, u1, sumi0_1);


    // --- 第二次展开 (l0 = 2, l1 = 0) ---
    const int u2 = __ldg(&q8_ptr[base_idx + 2]);
    const int u3 = __ldg(&q8_ptr[base_idx + 3]);

    const int qhl_1 = (bq1->qh[iqs] >> 4) & 0x0F;
    // const uint64_t grid_val_1 = shared_iq1s_grid[qs[2 + iqs] | (qhl_1 << 8)];
    const uint64_t grid_val_1 = __ldg(&iq1s_grid_gpu_new[qs[2 + iqs] | (qhl_1 << 8)]);
    const int grid1_0 = (int)(grid_val_1 & 0xFFFFFFFF);
    const int grid1_1 = (int)(grid_val_1 >> 32);

    sumi0_0 = __dp4a(grid1_0, u2, sumi0_0);
    sumi0_1 = __dp4a(grid1_1, u3, sumi0_1);


    // --- 第三次展开 (l0 = 4, l1 = 1) ---
    const int u4 = __ldg(&q8_ptr[base_idx + 4]);
    const int u5 = __ldg(&q8_ptr[base_idx + 5]);

    const int qhl_2 = bq1->qh[4 + iqs] & 0x0F; // 注意：l1=1，索引偏移 4
    // const uint64_t grid_val_2 = shared_iq1s_grid[qs[4 + iqs] | (qhl_2 << 8)];
    const uint64_t grid_val_2 = __ldg(&iq1s_grid_gpu_new[qs[4 + iqs] | (qhl_2 << 8)]);
    const int grid2_0 = (int)(grid_val_2 & 0xFFFFFFFF);
    const int grid2_1 = (int)(grid_val_2 >> 32);
    
    sumi1_0 = __dp4a(grid2_0, u4, sumi1_0);
    sumi1_1 = __dp4a(grid2_1, u5, sumi1_1);


    // --- 第四次展开 (l0 = 6, l1 = 1) ---
    const int u6 = __ldg(&q8_ptr[base_idx + 6]);
    const int u7 = __ldg(&q8_ptr[base_idx + 7]);

    const int qhl_3 = (bq1->qh[4 + iqs] >> 4) & 0x0F;
    // const uint64_t grid_val_3 = shared_iq1s_grid[qs[6 + iqs] | (qhl_3 << 8)];
    const uint64_t grid_val_3 = __ldg(&iq1s_grid_gpu_new[qs[6 + iqs] | (qhl_3 << 8)]);
    const int grid3_0 = (int)(grid_val_3 & 0xFFFFFFFF);
    const int grid3_1 = (int)(grid_val_3 >> 32);

    sumi1_0 = __dp4a(grid3_0, u6, sumi1_0);
    sumi1_1 = __dp4a(grid3_1, u7, sumi1_1);


    // 3. 循环外统一浮点计算与缩放
    const int sumi_l0 = sumi0_0 + sumi0_1;
    const int sumi_l1 = sumi1_0 + sumi1_1;

    const float d1q = dx * (bq1->scales[iqs] & 0xf);
    const float d2q = dx * (bq1->scales[iqs] >> 4);
    
    // 【核心魔法还原】：预先在表里放大了 8 倍，最后乘 0.125f 还原！
    return bq8_1[0].d * ( (sumi_l0 * 0.125f) * d1q + (sumi_l1 * 0.125f) * d2q );
#endif
}
// 注意：函数签名中 shared_iq1s_grid 需要改为接受两个 uint32_t 指针
static __device__ __forceinline__ float vec_dot_iq1_m_r4_q8_k128_shared_double(
    const void * __restrict__ vbq, const block_q8_K128 * __restrict__ bq8_1, 
    const int & iqs, const int & iqa, const float & dx, 
    const uint32_t * __restrict__ shared_grid0, // 替换为分离的数组
    const uint32_t * __restrict__ shared_grid1) 
{
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
    const block_iq1_m_r4 * bq1 = (const block_iq1_m_r4 *) vbq;
    const uint8_t * qs        = bq1->qs;
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        ...
        // 计算索引 (保持不变)
        const int lookup_idx = qs[l0*2+iqs] | ((qhl & 0x07) << 8);

        // --- 修改点 3：分别读取预处理好的 32-bit 数据，跳过中间位运算 ---
        const int grid0 = shared_grid0[lookup_idx];
        const int grid1 = shared_grid1[lookup_idx];
...
static __device__ __forceinline__ float vec_dot_iq1_s_r4_q8_k128(
    const void * __restrict__ vbq, const block_q8_K128 * __restrict__ bq8_1, const int & iqs, const int & iqa, const float & dx) {
#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 610 || defined USE_ROCM
    const block_iq1_s_r4 * bq1 = (const block_iq1_s_r4 *) vbq;
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        const int grid = iq1s_grid_gpu[qs[l0*2+iqs] | (((qh >> 3*(l0/2)) & 0x07) << 8)];
