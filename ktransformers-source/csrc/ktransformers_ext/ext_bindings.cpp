...
#include "operators/llamafile/flexmoe.h"
  ...
  public:
    class WarmUpBindinds {
      public:
        ...
        static void inner(void *args) {
            Args *args_ = (Args *)args;
            args_->cpuinfer->enqueue(&FlexMOE::warm_up, args_->moe);
        }
        static std::pair<intptr_t, intptr_t> cpuinfer_interface(FlexMOE &moe) {
            Args *args = new Args{nullptr, &moe};
            return std::make_pair((intptr_t)&inner, (intptr_t)args);
        }
    };
    class ForwardBindings {
      public:
        ...
        static void inner(void *args) {
            Args *args_ = (Args *)args;
            args_->cpuinfer->enqueue(
                &FlexMOE::forward, args_->moe, args_->qlen, args_->k,
                args_->expert_ids, args_->weights, args_->input, args_->output, args_->batch_size_tensor);
        }
        static std::pair<intptr_t, intptr_t>
        cpuinfer_interface(FlexMOE &moe, int qlen, int k, intptr_t expert_ids,
                           intptr_t weights, intptr_t input, intptr_t output, intptr_t batch_size_tensor) {
            Args *args = new Args{nullptr,
                                  &moe,
                                  qlen,
    ...
    class ForwardFlexBindings {
      public:
        struct Args {
            ...
            const float *weights;
            const void *input;
            const void *w12_projs;
            void *output;
        };
        static void inner(void *args) {
            Args *args_ = (Args *)args;
            args_->cpuinfer->enqueue(
                &FlexMOE::forward_flex, args_->moe, args_->qlen, args_->k,
                args_->expert_ids, args_->weights, args_->input, args_->w12_projs, args_->output);
        }
        static std::pair<intptr_t, intptr_t>
        cpuinfer_interface(FlexMOE &moe, int qlen, int k, intptr_t expert_ids,
                           intptr_t weights, intptr_t input, intptr_t w12_projs,
                           intptr_t output) {
            Args *args = new Args{nullptr,
                                  &moe,
                                  qlen,
    ...
    class ForwardFlexManyBindings {
        public:
          ...
          static void inner(void *args) {
              Args *args_ = (Args *)args;
              args_->cpuinfer->enqueue(
                  &FlexMOE::forward_flex_many, args_->moe, args_->qlen, args_->k,
                  args_->expert_ids, args_->weights, args_->input, args_->w12_projs, args_->output, args_->batch_size_tensor);
          }
          static std::pair<intptr_t, intptr_t>
          cpuinfer_interface(FlexMOE &moe, int qlen, int k, intptr_t expert_ids,
                             intptr_t weights, intptr_t input, intptr_t w12_projs,
                             intptr_t output,  intptr_t batch_size_tensor) {
              Args *args = new Args{nullptr,
                                    &moe,
                                    qlen,
                                    k,
      ...
      class ForwardFlexManyV1Bindings {
        public:
          struct Args {
              CPUInfer *cpuinfer;
              FlexMOE *moe;
          ...
          static void inner(void *args) {
            Args *args_ = (Args *)args;
            args_->cpuinfer->enqueue(
                &FlexMOE::forward_flex_many_v1, args_->moe, args_->qlen, args_->k, args_->max_k,
                args_->expert_ids, args_->weights, args_->input, args_->w12_projs, args_->output, args_->batch_size_tensor);
          }
          static std::pair<intptr_t, intptr_t>
          cpuinfer_interface(FlexMOE &moe, int qlen, intptr_t k, int max_k, intptr_t expert_ids,
                             intptr_t weights, intptr_t input, intptr_t w12_projs, intptr_t output, intptr_t batch_size_tensor) {
              Args *args = new Args{nullptr,
                                    &moe,
                                    qlen,
                                    (const uint64_t *)k,
      ...
      class ForwardManyFlexBindings {
        public:
          ...
          static void inner(void *args) {
              Args *args_ = (Args *)args;
              args_->cpuinfer->enqueue(
                  &FlexMOE::forward_many_flex, args_->moe, args_->qlen, args_->k,
                  args_->expert_ids, args_->weights, args_->input, args_->w12_projs, args_->output, args_->batch_size_tensor);
          }
          static std::pair<intptr_t, intptr_t>
          cpuinfer_interface(FlexMOE &moe, int qlen, int k, intptr_t expert_ids,
      ...
      class ForwardManyFlexV1Bindings {
        public:
          struct Args {
              CPUInfer *cpuinfer;
              FlexMOE *moe;
          ...
          static void inner(void *args) {
              Args *args_ = (Args *)args;
              args_->cpuinfer->enqueue(
                  &FlexMOE::forward_many_flex_v1, args_->moe, args_->qlen, args_->k, args_->max_k,
                  args_->expert_ids, args_->weights, args_->input, args_->w12_projs, args_->output, args_->batch_size_tensor);
          }
          static std::pair<intptr_t, intptr_t>
          cpuinfer_interface(FlexMOE &moe, int qlen, intptr_t k, int max_k,intptr_t expert_ids,
...
PYBIND11_MODULE(cpuinfer_ext, m) {
    ...
    auto flexmoe_module = m.def_submodule("flexmoe");
    py::class_<FlexMOEConfig>(flexmoe_module, "FlexMOEConfig")
        .def(py::init([](int expert_num, int routed_expert_num, int flex_topk,int hidden_size,
                         int intermediate_size, int stride, int group_min_len,
                         int group_max_len, intptr_t gate_proj,
                         intptr_t up_proj, intptr_t down_proj, 
                         int gate_type, int gate_ori_type, int gate_res_type,
                         int up_type, int up_ori_type, int up_res_type,
                         int down_type, int down_ori_type, int down_res_type,
                         int hidden_type) {
            return FlexMOEConfig(expert_num, routed_expert_num, flex_topk, hidden_size,
                             intermediate_size, stride, group_min_len,
                             group_max_len, (void *)gate_proj, (void *)up_proj,
                             (void *)down_proj, 
    ...
    py::class_<FlexMOE>(flexmoe_module, "FlexMOE")
        .def(py::init<FlexMOEConfig>())
        .def("warm_up", &FlexMOEBindings::WarmUpBindinds::cpuinfer_interface)
        .def("forward", &FlexMOEBindings::ForwardBindings::cpuinfer_interface)
        .def("forward_flex", &FlexMOEBindings::ForwardFlexBindings::cpuinfer_interface)
        .def("forward_flex_many", &FlexMOEBindings::ForwardFlexManyBindings::cpuinfer_interface)
        .def("forward_flex_many_v1", &FlexMOEBindings::ForwardFlexManyV1Bindings::cpuinfer_interface)
        .def("forward_many_flex", &FlexMOEBindings::ForwardManyFlexBindings::cpuinfer_interface)
        .def("forward_many_flex_v1", &FlexMOEBindings::ForwardManyFlexV1Bindings::cpuinfer_interface);
