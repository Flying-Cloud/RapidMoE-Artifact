...
void MLP::forward_many(int qlen, const void* input, void* output, Backend* backend) {
    ...
    backend->do_work_stealing_job(nth, nullptr, [&](int task_id) {
        int ith = task_id;
        void* gate_proj_ptr = (uint8_t*)gate_proj_ + ith * config_.stride * ggml_row_size(config_.gate_type, config_.hidden_size);
        ...
        void* up_proj_ptr = (uint8_t*)up_proj_ + ith * config_.stride * ggml_row_size(config_.up_type, config_.hidden_size);
    ...
    backend->do_work_stealing_job(nth, nullptr, [&](int task_id) {
        int ith = task_id;
        void* down_proj_ptr = (uint8_t*)down_proj_ + ith * config_.stride * ggml_row_size(config_.down_type, config_.intermediate_size);
