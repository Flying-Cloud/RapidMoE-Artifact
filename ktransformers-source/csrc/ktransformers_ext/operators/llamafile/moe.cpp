...
void MOE::forward_one(int k, const uint64_t* expert_ids, const float* weights, const void* input, void* output, Backend* backend) {
    ...
    backend->do_work_stealing_job(nth * k, nullptr, [&](int task_id) {
        ...
        void* gate_proj_ptr = (uint8_t*)gate_proj_numa_[Backend::numa_node] + (expert_id * config_.intermediate_size + ith * config_.stride) * ggml_row_size(config_.gate_type, config_.hidden_size);
        #else
        ...
        void*  up_proj_ptr = (uint8_t*)up_proj_numa_[Backend::numa_node] +(expert_id * config_.intermediate_size + ith * config_.stride) * ggml_row_size(config_.up_type, config_.hidden_size);
        #else
        void* up_proj_ptr = (uint8_t*)up_proj_ + (expert_id * config_.intermediate_size + ith * config_.stride) * ggml_row_size(config_.up_type, config_.hidden_size);
        #endif
    ...
    backend->do_work_stealing_job(nth, nullptr, [&](int task_id) {
        ...
        for (int expert_idx = 0; expert_idx < k; expert_idx++) {
            ...
            void*down_proj_ptr = (uint8_t*)down_proj_numa_[Backend::numa_node] +(expert_id * config_.hidden_size + ith * config_.stride) * ggml_row_size(config_.down_type, config_.intermediate_size);
            #else
...
void MOE::forward_many(int qlen, int k, const uint64_t* expert_ids, const float* weights, const void* input, void* output, Backend* backend) {
    ...
    backend->do_work_stealing_job(nth * config_.expert_num, nullptr, [&](int task_id) {
        ...
        void* gate_proj_ptr = (uint8_t*)gate_proj_numa_[Backend::numa_node] + (expert_idx * config_.intermediate_size + ith * stride) * ggml_row_size(config_.gate_type, config_.hidden_size);
        #else
        ...
        void* up_proj_ptr = (uint8_t*)up_proj_numa_[Backend::numa_node] +  (expert_idx * config_.intermediate_size + ith * stride) * ggml_row_size(config_.up_type, config_.hidden_size);
        #else
        void* up_proj_ptr = (uint8_t*)up_proj_ + (expert_idx * config_.intermediate_size + ith * stride) * ggml_row_size(config_.up_type, config_.hidden_size);
        #endif
    ...
    backend->do_work_stealing_job(nth * config_.expert_num, nullptr, [&](int task_id) {
        ...
        void* down_proj_ptr = (uint8_t*)down_proj_numa_[Backend::numa_node] + (expert_idx * config_.hidden_size + ith * stride) * ggml_row_size(config_.down_type, config_.intermediate_size);
        #else
