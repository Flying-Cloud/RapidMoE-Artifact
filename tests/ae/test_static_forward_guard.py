#!/usr/bin/env python3
"""Verify the capture-safe static r=1 device-copy guard."""
...
expert.static_r_value = torch.full((1,), 1, dtype=torch.int32, device=device)
...
assert expert.flex_decode_topk.item() == 1
assert expert.flex_decode_idx.item() == 1
assert expert.flex_prefill_topk.item() == 6

expert.force_static_r(4)
assert expert.flex_prefill_topk.item() == 1
assert expert.flex_prefill_idx.item() == 1
...
print("[PASS] static decode/prefill guards copy device-side r=1 in place")
print("[PASS] static r=1 device copies execute on every CUDA Graph replay")
